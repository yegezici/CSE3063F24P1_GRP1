...
class TestStudentInterface(unittest.TestCase):
    ...
    def test_register_course(self):
        """Test if registerCourse works as expected"""
        # Ensure that when registering a course, it gets added to the student's transcript
        available_courses = self.student_interface.show_registrable_courses()
        selected_course = available_courses[0]  # Get the first available course
        #self.student_interface.register_course()  # Simulate registering the course
        
        # Check if the course is in the student's transcript (in the waited courses list)
        self.assertIn(selected_course, self.student.get_transcript().get_waited_courses(), "The selected course should be added to the student's waited courses list.")
