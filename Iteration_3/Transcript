class Transcript:
    def __init__(self, completed_courses=None, current_courses=None, waited_courses=None, 
        ...
        self.waited_courses = waited_courses or []
        self.current_sections = current_sections or self.add_course_sections(self.current_courses)
        self.waited_sections = waited_sections or self.add_course_sections(self.waited_courses)
    ...
    def add_waited_course(self, course):
        if course:
            self.waited_courses.append(course)
        else:
            print("The course object is null")

    def delete_from_waited_course(self, course):
        if course in self.waited_courses:
            self.waited_courses.remove(course)
        else:
            print("The course object is null")

    def add_current_section(self, course_section):
        if course_section:
            self.current_sections.append(course_section)
        else:
            print("The course section object is null")

    def delete_from_waited_sections(self, course_section):
        if course_section in self.waited_sections:
            self.waited_sections.remove(course_section)
        else:
            print("The course section object is null")

    def show_waited_courses(self):
        print("\nCourses that are waiting for approval listed below:")
        for i, course in enumerate(self.waited_courses, start=1):
    ...
    def show_completed_courses(self):
        ...
        for i, course in enumerate(self.completed_courses, start=1):
    ...
    def show_current_courses(self):
        ...
        for i, course in enumerate(self.current_courses, start=1):
            print(f"{i:3}-   {course.get_course_id():<10} {course.get_course_name():<50} {course.get_credits()}")
    ...
    def set_waited_courses(self, waited_courses):
        self.waited_courses = waited_courses
    ...
    def get_waited_sections(self):
        return self.waited_sections

    def set_waited_sections(self, waited_sections):
        self.waited_sections = waited_sections
